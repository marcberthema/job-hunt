...
For a selected posting, refresh it read-only, re-check duplicates, and follow the `addjob` skill: append one row to `applications.md` with `Status: New`. When the client is anonymous, use `procom` as the company slug unless the description reveals the real client. Never submit an application or change a row's status to `Applied` or `Rejected` without the user's explicit confirmation.
